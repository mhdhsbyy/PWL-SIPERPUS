    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="post" action="{{ route('loans.update', $loan->id ) }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                    @csrf
                    @method('patch')
                    <div class="max-w-xl">
                        <x-input-label for="user_npm" value="Peminjam"/>
                        <x-text-input id="user_npm" type="text" name="user_npm" class="mt-1 block w-full" value="{{ old('user_npm', $loan->user_npm) }}" required/>
                        <x-input-error class="mt-2" :messages="$errors->get('user_npm')" />
                    </div>

                    <div class="max-w-xl">
                        <x-input-label for="loan_at" value="Taggal Pinjam"/>
